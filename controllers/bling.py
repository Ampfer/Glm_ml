...
def bling_lieto_clientes(cliente_bl,vendedor):
	...
	cliente.nomcli = remover_acentos1(cliente_bl['nome'][:50]).upper()
	cliente.nomfan = ''
	cliente.fisjur = 'J' if len(cnpj) == 13 else 'F'
	cliente.endcli = remover_acentos1(cliente_bl['endereco'])[:50].upper()
	cliente.baicli = remover_acentos1(cliente_bl['bairro'])[:35].upper()
	cliente.cidcli = remover_acentos1(cliente_bl['cidade'])[:35].upper()
	cliente.estcli = remover_acentos1(cliente_bl['uf'][:2]).upper()

...
Clientes = db.define_table('clientes',
	...
	Field('codcid','string',label='Cod.Cidade:',length=20),
...
Pedidos = db.define_table('pedidos',
	...
	Field.Virtual('taxa',lambda row: totalTaxas(row), label='Tarifa:'),
	Field('status', 'string', label='Status:', length=30),
	Field('pagamento', 'string', label='Pagamento:', length=30),
	...
	Field('receber','string',label='Receber:', length=1),
	Field('nota','integer',label='Nota:'),
	Field('valpag','decimal(7,2)',label='Vl.Pago:'),
    )
